...
function renderHist() {
  ...
  el.innerHTML = filtered.map(os => {
    ...
    const destaque = (txt) => {
      ...
      return esc(txt).replace(re, '<mark class="busca-hl">$1</mark>');
    };
    const eqInfo = [os.eqMarca, os.eqModelo].filter(Boolean).join(' ') +
      (os.eqSerie ? ' <span class="os-serie">S/N: ' + destaque(os.eqSerie) + '</span>' : '') +
      (os.horimetro ? ' — ' + os.horimetro + 'h' : '') +
      ' — Tec: ' + esc(os.tecNome || '--');
    return '<div class="os-card' + (isAgendado ? ' os-card-agendado' : '') + '">' +
      '<div class="os-card-top"><div class="os-card-info">' +
