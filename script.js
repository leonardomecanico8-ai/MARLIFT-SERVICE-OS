...
function switchTab(id) {
  ...
  if (id === 'tab-cfg') { carregarCfg(); renderClientList(); atualizarSyncStatusBar(); }
...
/* Oculta/mostra os cards de dados do cliente (recolher) */
function mostrarCardCliente(show) {
  const card = document.getElementById('cardDadosCliente');
...
function carregarCliente(id) {
  ...
  // Preencher campos cliente
  f('cNome', c.nome); f('cCnpj', c.cnpj || ''); f('cEnd', c.end);
  f('cContato', c.contato || ''); f('cTel', c.tel || ''); f('cEmail', c.email || '');

  // Preencher campos equipamento automaticamente
  const marcaEl = document.getElementById('eMarca');
  if (marcaEl) {
    // tenta achar a option com o mesmo valor
    let found = false;
    Array.from(marcaEl.options).forEach(opt => {
      if (opt.value.toUpperCase() === (c.marca || '').toUpperCase()) {
        marcaEl.value = opt.value;
        found = true;
      }
    });
    if (!found) {
      // adiciona opção temporária caso a marca não esteja na lista
      const opt = document.createElement('option');
      opt.value = c.marca; opt.textContent = c.marca;
      marcaEl.appendChild(opt);
      marcaEl.value = c.marca;
    }
  }
  f('eModelo', c.modelo);
  const combEl = document.getElementById('eComb');
  if (combEl) {
    Array.from(combEl.options).forEach(opt => {
      if (opt.value.toUpperCase() === (c.comb || '').toUpperCase()) combEl.value = opt.value;
    });
  }
  f('eSerie', c.serie);

  // Mostrar badge de cliente carregado
  const badge = document.getElementById('clienteCarregadoBadge');
  const nomeSpan = document.getElementById('clienteCarregadoNome');
  if (badge) badge.style.display = 'flex';
  if (nomeSpan) nomeSpan.textContent = c.nome + ' — ' + c.marca + ' ' + c.modelo;

  // Revelar cards na sequência
  ['cardDadosCliente','cardTecnico','cardEquipamento','cardTriagem'].forEach(cid => {
    const el = document.getElementById(cid);
    if (el) el.style.display = 'block';
  });

  autoSave(); prog();
  toast('"' + c.nome + '" carregado! Preencha a triagem.', 'success');

  // Scroll suave até a triagem
  setTimeout(() => {
    const triagem = document.getElementById('cardTriagem');
    if (triagem) triagem.scrollIntoView({ behavior: 'smooth', block: 'start' });
  }, 250);
}
...
function salvarCliente() {
  ...
  const marca = g('cadMarca').trim();
  const modelo = g('cadModelo').trim();
  const serie = g('cadSerie').trim();
  ...
  const novo = {
    ...
    marca, modelo, comb: g('cadComb'), serie
  };
  clientes.push(novo);
  salvarClientes();
  atualizarSelectCli();
  renderClientList();
  ['cadNome','cadCnpj','cadEnd','cadContato','cadEmail','cadTel','cadMarca','cadModelo','cadSerie'].forEach(id => f(id, ''));
  fecharModal();
  ...
  if (cfg.googleScript) {
    if (navigator.onLine) {
      fetch(cfg.googleScript, {
        method: 'POST',
        mode: 'no-cors',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ action: 'saveCliente', cliente: novo })
      }).catch(() => adicionarNaFila('cliente', novo));
    } else {
      adicionarNaFila('cliente', novo);
    }
  }
}
...
function editarCliente(id) {
  ...
  f('editCliModelo', c.modelo);
  f('editCliSerie', c.serie);
  // Set selects
  const selMarca = document.getElementById('editCliMarca');
  if (selMarca) selMarca.value = c.marca || '';
  const selComb = document.getElementById('editCliComb');
  if (selComb) selComb.value = c.comb || 'GLP';
...
function salvarEdicaoCliente() {
  ...
  const marca = g('editCliMarca').trim();
  const modelo = g('editCliModelo').trim();
  const serie = g('editCliSerie').trim();
  if (!nome || !end || !marca || !modelo || !serie) {
    toast('Preencha os campos obrigatórios (*)', 'error');
    return;
  }
  ...
  clientes[idx] = {
    ...
    tel: g('editCliTel'), marca, modelo,
    comb: g('editCliComb'), serie
  };
  salvarClientes();
  atualizarSelectCli();
  renderClientList();
...
function renderClientList() {
  ...
  clientes.slice().sort((a, b) => a.nome.localeCompare(b.nome)).forEach(c => {
    ...
    div.innerHTML =
      ...
      '<div class="client-meta">' + esc(c.marca) + ' ' + esc(c.modelo) + ' - S/N: ' + esc(c.serie) + '</div></div>' +
...
function carregarOSParaAtendimento(id) {
  ...
  if (marcaEl) {
    let found = false;
    Array.from(marcaEl.options).forEach(opt => {
      if (opt.value.toUpperCase() === (os.eqMarca || '').toUpperCase()) { marcaEl.value = opt.value; found = true; }
    });
    if (!found) {
      const opt = document.createElement('option');
      opt.value = os.eqMarca; opt.textContent = os.eqMarca;
      marcaEl.appendChild(opt); marcaEl.value = os.eqMarca;
    }
  }
  f('eModelo', os.eqModelo || '');
  const combEl = document.getElementById('eComb');
  if (combEl) Array.from(combEl.options).forEach(opt => {
    if (opt.value.toUpperCase() === (os.eqComb || '').toUpperCase()) combEl.value = opt.value;
  });
  f('eSerie', os.eqSerie || ''); f('eHor', os.horimetro || '');

  const tipoEl = document.getElementById('tipo');
  if (tipoEl) {
    Array.from(tipoEl.options).forEach(opt => { if (opt.value === os.tipoChamado) tipoEl.value = opt.value; });
  }
  const priorEl = document.getElementById('prior');
  if (priorEl) {
    Array.from(priorEl.options).forEach(opt => { if (opt.value === os.prioridade) priorEl.value = opt.value; });
  }
  f('defeito', os.defeito || '');
  f('agendaData', os.agendaData || ''); f('agendaHora', os.agendaHora || ''); f('agendaObs', os.agendaObs || '');

  // Mostrar badge de cliente
  const badge = document.getElementById('clienteCarregadoBadge');
  const nomeSpan = document.getElementById('clienteCarregadoNome');
  if (badge) badge.style.display = 'flex';
  if (nomeSpan) nomeSpan.textContent = (os.cliNome || '') + ' — ' + (os.eqMarca || '') + ' ' + (os.eqModelo || '');

  // Revelar todos os cards incluindo os de trabalho
  ['cardDadosCliente','cardTecnico','cardEquipamento','cardTriagem',
   'cardCronometro','cardRelatorio','cardFotos','cardEncerramento'].forEach(cid => {
    const el = document.getElementById(cid);
    if (el) el.style.display = 'block';
  ...
  // Marcar status como Pendente (em andamento)
  const statusEl = document.getElementById('status');
...
function renderHist() {
  ...
  if (!el) return;
  const hist = JSON.parse(localStorage.getItem('os_historico') || '[]');
  const q = (g('busca') || '').toLowerCase();
  const filtered = q ? hist.filter(os =>
  ...
  if (filtered.length === 0) {
    el.innerHTML = '<div class="os-empty"><i class="fa-solid fa-magnifying-glass"></i><p>' + (q ? 'Nenhum resultado encontrado.' : 'Nenhuma OS registrada.') + '</p></div>';
    return;
  }
  const statusBadge = {
    'Concluído': 'badge-green', 'Concluido': 'badge-green',
  ...
  el.innerHTML = filtered.map(os => {
    ...
    return '<div class="os-card' + (isAgendado ? ' os-card-agendado' : '') + '">' +
      ...
      '<div class="os-cliente">' + esc(os.cliNome) + '</div>' +
      '<div class="os-meta">' + esc(os.eqMarca || '') + ' ' + esc(os.eqModelo || '') + (os.horimetro ? ' — ' + os.horimetro + 'h' : '') + ' — Tec: ' + esc(os.tecNome || '--') + '</div>' +
      '<div class="os-tags">' +
...
function editarOS(id) {
  ...
  if (!os) return;
  editingOSId = id;
  const body = document.getElementById('editOSBody');
  if (!body) return;
  body.innerHTML =
    '<div class="edit-field"><label>Cliente</label><input type="text" id="editCli" value="' + esc(os.cliNome || '') + '"></div>' +
    '<div class="edit-field"><label>Técnico</label><input type="text" id="editTec" value="' + esc(os.tecNome || '') + '"></div>' +
    '<div class="edit-field"><label>Defeito</label><input type="text" id="editDef" value="' + esc(os.defeito || '') + '"></div>' +
    '<div class="edit-field"><label>Serviço Executado</label><textarea id="editServ">' + esc(os.servico || '') + '</textarea></div>' +
    '<div class="edit-field"><label>Peças</label><textarea id="editPecas">' + esc(os.pecas || '') + '</textarea></div>' +
    '<div class="edit-field"><label>Observações</label><textarea id="editObs">' + esc(os.obs || '') + '</textarea></div>' +
    '<div class="edit-field"><label>Status</label><select id="editStatus">' +
    ['Concluído','Pendente','Aguardando Peça','Orçamento Enviado'].map(s =>
...
function _doPDF(os) {
  ...
  if (os.assinatura && os.assinatura.img) {
    try { doc.addImage(os.assinatura.img, 'PNG', M + 3, y + 9, 65, 20); } catch(ex) {}
  } else {
